...
class Model_reseller extends CI_model{
    ...
    function penjualan_list_konsumen($id,$level){
        return $this->db->query("SELECT * FROM `rb_penjualan` a LEFT JOIN rb_konsumen b ON a.id_pembeli=b.id_konsumen where a.status_penjual='$level' AND a.id_penjual='$id' ORDER BY a.id_penjualan DESC");
    }
    ...
    function jual_reseller($penjual, $produk,$varian=null){
        $kondisi = "";
        if(!empty($varian)){
            $kondisi = "AND b.id_varian = '$varian'";
        }
        return $this->db->query("SELECT sum(jumlah) as jual FROM `rb_penjualan` a JOIN rb_penjualan_detail b ON a.id_penjualan=b.id_penjualan where a.status_pembeli='konsumen' AND a.status_penjual='reseller' AND a.id_penjual='$penjual' AND b.id_produk='$produk' AND a.proses='1' $where");
    }

    function beli_reseller($pembeli, $produk,$varian=null){
        $kondisi = "";
        if(!empty($varian)){
            $kondisi = "AND b.id_varian = '$varian'";
        }
        return $this->db->query("SELECT sum(jumlah) as beli FROM `rb_penjualan` a JOIN rb_penjualan_detail b ON a.id_penjualan=b.id_penjualan where a.status_pembeli='reseller' AND a.status_penjual='admin' AND a.id_pembeli='$pembeli' AND b.id_produk='$produk' AND a.proses='1' $kondisi ");
    }

    function jual_reseller_varian($penjual, $produk,$varian=null){
        if(!empty($varian)){
            $kondisi = "AND b.id_varian = '$varian'";
        }else{
            $kondisi = "group by id_varian";
        }
        return $this->db->query("SELECT sum(jumlah) as jual,id_varian FROM `rb_penjualan` a JOIN rb_penjualan_detail b ON a.id_penjualan=b.id_penjualan where a.status_pembeli='konsumen' AND a.status_penjual='reseller' AND a.id_penjual='$penjual' AND b.id_produk='$produk' AND a.proses='1' $kondisi");
    }

    function beli_reseller_varian($pembeli, $produk,$varian=null){
        if(!empty($varian)){
            $kondisi = "AND b.id_varian = '$varian'";
        }else{
            $kondisi = "group by id_varian";
        }
        return $this->db->query("SELECT sum(jumlah) as beli,id_varian FROM `rb_penjualan` a JOIN rb_penjualan_detail b ON a.id_penjualan=b.id_penjualan where a.status_pembeli='reseller' AND a.status_penjual='admin' AND a.id_pembeli='$pembeli' AND b.id_produk='$produk' AND a.proses='1' $kondisi ");
    }
    ...
    function penjualan_konsumen_detail_reseller($id){
        return $this->db->query("SELECT * FROM `rb_penjualan` a LEFT JOIN rb_konsumen b ON a.id_pembeli=b.id_konsumen where a.id_penjualan='$id'");
    }
