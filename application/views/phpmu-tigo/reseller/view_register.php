...
<li role='presentation' class='<?php if(!$this->session->flashdata('notifdaftar')){ ?>active<?php }?>'><a href='#konsumen' id='konsumen-tab' role='tab' data-toggle='tab' aria-controls='konsumen' aria-expanded='true'>Pendaftaran Pembeli </a></li>
...
<li role='presentation' class='<?php if($this->session->flashdata('notifdaftar')){ ?>active<?php } ?>'><a href='#kontributor' role='tab' id='reseller-tab' data-toggle='tab' aria-controls='reseller' aria-expanded='false'>Pendaftaran Kontributor</a></li>
    </ul><br>
    <div id='myTabContent' class='tab-content'>
<div role='tabpanel' class='tab-pane fade <?php if(!$this->session->flashdata('notifdaftar')){ ?>active<?php }?> in' id='konsumen' aria-labelledby='konsumen-tab'>
        ...
        <div class='alert alert-warning'><b>PENTING!</b> Lengkapi Form dibawah ini untuk mendaftarkan diri sebagai <b>Pelapak</b>, harap di isi dengan data yang sebenar-benarnya, Terima kasih...</div>
         ...
         <div role='tabpanel' class='tab-pane <?php if($this->session->flashdata('notifdaftar')){ ?>active <?php }?> fade in' id='kontributor' aria-labelledby='kontributor-tab'>
        <?php if(!$this->session->flashdata('notifdaftar')){ ?>
        <div class='alert alert-danger'><b>PENTING!</b> Lengkapi Form dibawah ini untuk mendaftarkan diri sebagai <b>Kontributor</b>, harap di isi dengan data yang sebenar-benarnya, Terima kasih...</div>
        <?php } ?>
        <?php if($this->session->flashdata('notifdaftar')){ ?>
        <div class='alert alert-success'><b><?php echo $this->session->flashdata('notifdaftar'); ?></b> </div>
        <?php } ?>
            <div class="block-content">
                <div id="writecomment">
                    <form action="<?php echo base_url(); ?>auth/register" method="POST" id="form_komentar">
                        ...
                        <p class="contact-form-user">
                            <label for="c_name">Nama Lengkap<span class="required">*</label>
                            <input type="text" name='c' placeholder="Tuliskan Lengkap Anda,.." class="required" required/>
                        </p>

                        <p class="contact-form-user">
                            <label for="c_name">No Handphone<span class="required">*</label>
                            <input type="number" name='d'placeholder="08**********" class="required" required/>
                        </p>


                        <p class="contact-form-email">
                            <label for="c_email">E-mail<span class="required">*</span></label>
                            <input type="email" name='e' placeholder="alamat.emailanda@mail.com" onkeyup="nospaces(this)" class="required" required/>
                        </p>

                        
                        
                        
                        <p><input type="submit" name="submit3" class="styled-button" value="Daftar Sebagai Kontributor"/></p>
                    </form>
                </div>
            </div>
            <div style='clear:both'><br></div>
