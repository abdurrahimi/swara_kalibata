    ...
    <div class='col-md-9' style='padding:0px'>
        ...
        <div class='col-md-9' style='padding:0px'>
            ...
            echo "<table class='table table-condensed' style='margin-bottom:0px'>
                ...
                if (!empty($varian))
                {
                    /*$varians = explode(";",$record['varian']);
                    $cvarian = count($varians);
                    echo "<tr><td style='font-weight:bold'>Varian</td><td>";
                    echo "<select name='varian' class='form-control'>";
                    for ($i=0;$i<$cvarian;$i++)
                    {
                        echo "<option value='".$varians[$i]."'>".$varians[$i]."</option>";
                    }
                    echo "</select>";
                    echo "</td></tr>";*/
                    /*echo "<tr><td style='font-weight:bold'>Varian</td><td>";
                    ...
                    foreach ($varian as $key => $value) {
                        echo "<option value='".$value->id_varian."'>".$value->nama_varian."<br/> <i style='font-size:10px;'> Stock :".$value->stock."</i></option>";
                    }
                    echo "</select>";
                    echo "</td></tr>";*/
                    echo "<tr><td style='font-weight:bold'>Varian</td><td>";
                    echo "<div class='row'><div class='col-md-12'>";
                    $jual_varian = $this->model_reseller->jual_reseller_varian($record['id_reseller'],$record['id_produk'])->result();
                    $beli_varian = $this->model_reseller->beli_reseller_varian($record['id_reseller'],$record['id_produk'])->result();
                    //var_dump($jual_varian);
                    
                    foreach ($varian as $key => $value) {
                        $stock_awal  = 0;
                        $stock_jual  = 0;
                        foreach ($beli_varian as  $b_varian) {
                            if($b_varian->id_varian<>$value->id_varian){
                                continue;
                            }
                            $stock_awal  += $b_varian->beli;

                        }
                        //echo $stock_awal;
                        foreach ($jual_varian as $j_varian) {

                            if($j_varian->id_varian<>$value->id_varian){
                                continue;
                            }
                            $stock_jual  += $j_varian->jual;

                        }
                        //echo "<br>".$stock_jual;

                        if(($stock_awal-$stock_jual)<1){
                            $color = "cfcfcf";
                            $colorstk ="d9534f";
                            $stocks = "Stok Produk Kosong";
                            $status = "disabled";
                        }else{
                            $color = "#000000";
                            $colorstk ="575757";
                            $stocks = ($stock_awal-$stock_jual)."&nbsp Stok Tersedia";
                            $status = "";
                        }
                        echo    '<div class="form-check form-check-inline" style="display: inline-block; margin-right: 20px;">
                                  <label class="form-check-label col-sm" for="inlineRadio'.$key.'" style="background-color: #fcf8e3;border-color: #faebcc; color:#'.$color.'; box-shadow: 0 4px 3px 0 rgba(2,2,2,0.2); padding:10px; margin-top:10px;">
                                  <input class="form-check-input" type="radio" name="id_varian" id="inlineRadio'.$key.'" value="'.$value->id_varian.'"'.$status.'>
                                  '.$value->nama_varian.'<br><i style="font-size:11px; font-weight:1; color:#'.$colorstk.'">'.$stocks.'</i></label>
                                </div>';
                        //echo "<input type='radio' class value='".$value->id_varian."'>".$value->nama_varian."<br/> <i style='font-size:10px;'> Stock :".$value->stock."</i></option>";
                    }
                    echo "</div></div>";
                    echo "</td></tr>";


                } else { echo "<input type='hidden' name='varian' value=''><input type='hidden' name='id_varian' value=''>"; }
