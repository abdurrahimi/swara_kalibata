...
class Auth extends CI_Controller {
	...
	public function register(){
		if (isset($_POST['submit1'])){
			...
			$cekemailk  = $this->model_app->view_where('rb_konsumen',array('email'=>$this->input->post('d')))->num_rows();
			$cekemailp  = $this->model_app->view_where('rb_reseller',array('email'=>$this->input->post('d')))->num_rows();
			...
			$this->session->set_flashdata('notifdaftar', 'Terima Kasih atas pendaftaran Anda, silahkan hubungi admin untuk mengaktifkan akun anda.');
	...
	public function lupass(){
		if (isset($_POST['lupa'])){
			...
			if ($total > 0){
				...
				$this->template->load(template().'/template',template().'/reseller/view_lupass_success',$data);
			}else{
				$data['email'] = $email;
