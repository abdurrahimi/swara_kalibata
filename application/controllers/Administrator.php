...
class Administrator extends CI_Controller {
	function index(){
		...
		if (isset($_POST['submit'])){
            if ($this->input->post() && (strtolower($this->input->post('security_code')) != strtolower($this->session->userdata('mycaptcha')))) {
                ...
                echo strtolower($this->input->post('security_code')).'|'.strtolower($this->session->userdata('mycaptcha'));
    ...
    function tambah_reseller(){
        ...
        if (isset($_POST['submit'])){
			...
			$cekusernamek  = $this->model_app->view_where('rb_konsumen',array('username'=>$this->input->post('a')))->num_rows();
			$cekusernamep  = $this->model_app->view_where('rb_reseller',array('username'=>$this->input->post('a')))->num_rows();
			$cekemailk  = $this->model_app->view_where('rb_konsumen',array('email'=>$this->input->post('g')))->num_rows();
			$cekemailp  = $this->model_app->view_where('rb_reseller',array('email'=>$this->input->post('g')))->num_rows();
            if ($cekusernamek > 0 || $cekusernamep > 0){
                $username = $this->input->post('a');
                echo "<script>window.alert('Maaf, Username $username sudah dipakai oleh orang lain!');
                                  window.location=('tambah_reseller')</script>";
            }
			if ($cekemailk > 0 || $cekemailp > 0){
    ...
    function edit_reseller(){
        ...
        if (isset($_POST['submit'])){
			$email = $this->input->post('g');
			$idr = $this->input->post('id');
            $cekemailp = $this->db->query("SELECT id_reseller FROM rb_reseller WHERE id_reseller != '$idr' AND email = '$email'")->num_rows();
			$cekemailk  = $this->model_app->view_where('rb_konsumen',array('email'=>$this->input->post('g')))->num_rows();
			if ($cekemailp > 0 || $cekemailk > 0)
			{echo "<script>window.alert('Maaf, Email $email sudah dipakai oleh orang lain!');
                                  window.location=('edit_reseller/$idr')</script>";
			}
            else {			
			$config['upload_path'] = 'asset/foto_user/';
            $config['allowed_types'] = 'gif|jpg|png|jpeg';
    ...
    // Controller Modul Dokter
    function dokter(){
        cek_session_akses('reseller',$this->session->id_session);
        $data['record'] = $this->model_app->view_ordering('rb_dokter','id_dokter','DESC');
        $this->template->load('administrator/template','administrator/additional/mod_dokter/view_dokter',$data);
    }
        ...
        if (isset($_POST['submit'])){
            $cek  = $this->model_app->view_where('rb_reseller',array('username'=>$this->input->post('a')))->num_rows();
                ...
                if ($hasil['file_name']==''){
                    $data = array('username'=>$this->input->post('a'),
                                'password'=>hash("sha512", md5($this->input->post('b'))),
                                'nama_dokter'=>$this->input->post('c'),
                                'jenis_kelamin'=>$this->input->post('d'),
                                'alamat_lengkap'=>$this->input->post('e'),
                                'no_telpon'=>$this->input->post('f'),
                                'spesialis'=>$this->input->post('g'));
                }else{
                    $data = array('username'=>$this->input->post('a'),
                                'password'=>hash("sha512", md5($this->input->post('b'))),
                                'nama_dokter'=>$this->input->post('c'),
                ...
                $this->model_app->insert('rb_dokter',$data);
                redirect('administrator/dokter');
                
            }
        }else{
            $this->template->load('administrator/template','administrator/additional/mod_dokter/view_dokter_tambah');
        }
    }
        ...
        if (isset($_POST['submit'])){
            ...
            if ($hasil['file_name']==''){
                if (trim($this->input->post('b')) != ''){
                   ...
                   $data = array('nama_dokter'=>$this->input->post('c'),
                                'jenis_kelamin'=>$this->input->post('d'),
                                'alamat_lengkap'=>$this->input->post('e'),
                                'no_telpon'=>$this->input->post('f'),
                                'spesialis'=>$this->input->post('g'));
            ...
            $where = array('id_dokter' => $this->input->post('id'));
            $this->model_app->update('rb_dokter', $data, $where);
            redirect('administrator/dokter');
			
			
        }else{
            $edit = $this->model_app->edit('rb_dokter',array('id_dokter'=>$id))->row_array();
            $data = array('rows' => $edit);
            $this->template->load('administrator/template','administrator/additional/mod_dokter/view_dokter_edit',$data);
        }
    }
    
    function delete_dokter(){
        cek_session_akses('reseller',$this->session->id_session);
        $id = array('id_dokter' => $this->uri->segment(3));
        $this->model_app->delete('rb_dokter',$id);
        redirect('administrator/dokter');
    }
    // SPK GEJALA
    function gejala(){
        cek_session_akses('reseller',$this->session->id_session);
        $data['record'] = $this->model_app->view_ordering('rb_gejala','kode_gejala','ASC');
        $this->template->load('administrator/template','administrator/additional/mod_gejala/view_gejala',$data);
    }

    function tambah_gejala(){
        cek_session_admin();
        if (isset($_POST['submit'])){
            $data = array('kode_gejala'=>$this->input->post('a'),
                          'nama_gejala'=>$this->input->post('b'));
            $this->model_app->insert('rb_gejala',$data);
            redirect($this->uri->segment(1).'/gejala');
        }else{
            $data['kode'] = $this->model_app->kode_spk('gejala','G','kode_gejala','rb_gejala');
            $this->template->load('administrator/template','administrator/additional/mod_gejala/view_gejala_tambah',$data);
        }
    }
    ...
    function edit_gejala(){
        ...
        if (isset($_POST['submit'])){
            $data = array('kode_gejala'=>$this->input->post('a'),
                          'nama_gejala'=>$this->input->post('b'));
            $where = array('id_gejala' => $this->input->post('id'));
            $this->model_app->update('rb_gejala', $data, $where);
            redirect($this->uri->segment(1).'/gejala');
        }else{
            $proses = $this->model_app->edit('rb_gejala', array('id_gejala' => $id))->row_array();
            $data = array('rows' => $proses);
            $this->template->load('administrator/template','administrator/additional/mod_gejala/view_gejala_edit',$data);
        }
    }

    function delete_gejala(){
        cek_session_admin();
        $id = array('id_gejala' => $this->uri->segment(3));
        $this->model_app->delete('rb_gejala',$id);
        redirect($this->uri->segment(1).'/gejala');
    }
    ...
    function solusi(){
        cek_session_akses('reseller',$this->session->id_session);
        $data['record'] = $this->model_app->view_ordering('rb_solusi','kode_solusi','ASC');
        $this->template->load('administrator/template','administrator/additional/mod_solusi/view_solusi',$data);
    }

    function tambah_solusi(){
        cek_session_admin();
        if (isset($_POST['submit'])){
            $data = array('kode_solusi'=>$this->input->post('a'),
                          'nama_solusi'=>$this->input->post('b'),
                          'solusi'=>$this->input->post('c'),
                          'definisi'=>$this->input->post('d'));
            $this->model_app->insert('rb_solusi',$data);
            redirect($this->uri->segment(1).'/solusi');
        }else{
            $data['kode'] = $this->model_app->kode_spk('solusi','P','kode_solusi','rb_solusi');
            $this->template->load('administrator/template','administrator/additional/mod_solusi/view_solusi_tambah',$data);
        }
    }
    ...
    function edit_solusi(){
        ...
        if (isset($_POST['submit'])){
            $data = array('kode_solusi'=>$this->input->post('a'),
                          ...
                          'solusi'=>$this->input->post('c'),
                          'definisi'=>$this->input->post('d'));
            $where = array('id_solusi' => $this->input->post('id'));
            $this->model_app->update('rb_solusi', $data, $where);
            redirect($this->uri->segment(1).'/solusi');
        }else{
            $proses = $this->model_app->edit('rb_solusi', array('id_solusi' => $id))->row_array();
            $data = array('rows' => $proses);
            $this->template->load('administrator/template','administrator/additional/mod_solusi/view_solusi_edit',$data);
        }
    }

    function delete_solusi(){
        cek_session_admin();
        $id = array('id_solusi' => $this->uri->segment(3));
        $this->model_app->delete('rb_solusi',$id);
        redirect($this->uri->segment(1).'/solusi');
    }
    ...
    function tambah_produk(){
        ...
        if (isset($_POST['submit'])){
            ...
            if (trim($fileName)!=''){
                ...
                $resize['source_image'] = './asset/foto_produk/'. $fileName;
