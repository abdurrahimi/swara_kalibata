...
class Members extends CI_Controller {
	...
	function edit_profile(){
		...
		if (isset($_POST['submit'])){
			...
			$idr = $this->session->id_konsumen;
			$cekemailp = $this->db->query("SELECT id_reseller FROM rb_reseller WHERE email = '$email'")->num_rows();
			$cekemailk  = $this->db->query("SELECT id_konsumen FROM rb_konsumen WHERE id_konsumen != '$idr' AND email = '$email'")->num_rows();
			if ($cekemailp > 0 || $cekemailk > 0)
			{echo "<script>window.alert('Maaf, Email $email sudah dipakai oleh orang lain!');
                                  window.location=('edit_profile')</script>";
			} else {
				...
				if ($reseller['id_penjual']==$id_reseller){
					if ($cek >=1){
						...
						$data = array('id_penjualan'=>$this->session->idp,
									  ...
									  'variasi'=>$this->input->post('varian'));
						//var_dump($data); exit;
					...
					if ($this->session->idp != ''){
						...
						$data['record'] = $this->db->join('rb_produk b','a.id_produk = b.id_produk')
										   ...
										   ->where('id_penjualan',$this->session->idp)
										   ->order_by('id_penjualan_detail','asc')
