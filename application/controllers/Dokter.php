...
class Dokter extends CI_Controller {
	function index(){
		if (isset($_POST['submit'])){
			...
			$cek = $this->db->query("SELECT * FROM rb_dokter where username='".$this->db->escape_str($username)."' AND password='".$this->db->escape_str($password)."'");
		    $row = $cek->row_array();
		    $total = $cek->num_rows();
			if ($total > 0){
				$this->session->set_userdata(array('id_dokter'=>$row['id_reseller'],
								   'username'=>$row['username'],
								   'level'=>'dokter'));
				redirect('dokter/home');
			}else{
				$data['title'] = 'Dokter &rsaquo; Log In';
				$this->load->view('dokter/view_login',$data);
			}
		}else{
			$data['title'] = 'Dokter &rsaquo; Log In';
			$this->load->view('dokter/view_login',$data);
	...
	function edit_penjualan(){
		...
		if (isset($_POST['submit1'])){
			if ($this->input->post('nama_pembeli') != ''){
				$data = array('kode_transaksi'=>$this->input->post('a'),
			        	  'nama_pembeli'=>$this->input->post('b'),
	...
	function pos(){
		...
		if (isset($_POST['selesai'])){
				...
				$this->model_app->insert('rb_penjualan',$data);
				$idp = $this->db->insert_id();
				$postemps = $this->model_app->view_where('rb_pos_temp',array('id_reseller'=>$this->session->id_reseller))->result();
				foreach ($postemps as $poss){
				...
				$idr = array('id_reseller' => $this->session->id_reseller);
		        $this->model_app->delete('rb_pos_temp',$idr);
				redirect($this->uri->segment(1).'/pos');
            ...
            if ($this->input->post('dd') > $stok){
		        ...
		        if ($cektemp->num_rows()<1){
					...
					foreach ($cektemp->result() as $temps) { $jumlahold = $temps->jumlah; }
					$jumlahnow = $jumlahold + $this->input->post('dd');
			        $data = array(
			        			  'jumlah'=>$jumlahnow)
			        			  ;
					$where = array('id_reseller' => $this->session->id_reseller,'id_produk'=>$this->input->post('aa'));
					$this->model_app->update('rb_pos_temp', $data, $where);
				}
				redirect($this->uri->segment(1).'/pos');
			}
	...
	function transaksi_penjualan_print(){
		...
		$jumbayar = str_replace(",","",$this->uri->segment(5));
		...
		$data['rows'] = $this->model_app->view_where('rb_reseller',array('id_reseller'=>$this->session->id_reseller))->row_array();
		$data['record'] = $this->db->query("SELECT a.*,b.nama_produk FROM rb_pos_temp a 
			LEFT JOIN rb_produk b ON a.id_produk = b.id_produk
			WHERE a.id_reseller = '$idreseller'")->result_array();
		$data['kode'] = $kode;
		$data['namapel'] = $nama;
		$data['jumlah_bayar'] = $jumbayar;
		$this->load->view('reseller/mod_pos/print_penjualan',$data);
	}

	function uang_kembali(){
		$total = str_replace(",","",$this->input->post('total_bayar'));
		$bayar = str_replace(",","",$this->input->post('jumlah_bayar'));
		$selisih = $bayar - $total;
		//echo rupiah($selisih);
		$data['selisih'] = $bayar - $total;
    	$this->load->view('reseller/mod_pos/view_bayar',$data);
    }

	function delete_pos_temp(){
        cek_session_reseller();
		$id = array('id_penjualan_detail' => $this->uri->segment(3));
		$this->model_app->delete('rb_pos_temp',$id);
		redirect($this->uri->segment(1).'/pos');
	}
