	...
	function tambah_produk(){
        ...
        if (isset($_POST['submit'])){
			...
			if(!empty($this->input->post('varian'))){
				$dataVarian['id_produk'] = $id_produk;
				...
				foreach ($this->input->post('varian') as $key => $value) {
					$dataVarian['nama_varian'] = $value;
					$dataVarian['stock'] = $this->input->post('stock')[$key];
					$this->model_app->insert('rb_produk_varian',$dataVarian);
					$id_varian = $this->db->insert_id();
					$data = array('id_penjualan'=>$idp,
		        			  'id_produk'=>$id_produk,
		        			  'jumlah'=>$this->input->post('stock')[$key],
