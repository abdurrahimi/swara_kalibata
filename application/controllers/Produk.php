...
class Produk extends CI_Controller {
				...
				if ($this->session->reseller==$id_reseller){
					...
					if ($this->session->idp != ''){
						...
						$data['record'] = $this->db->join('rb_produk b','a.id_produk = b.id_produk')
										   ...
										   ->where('session',$this->session->idp)
										   ->order_by('id_penjualan_detail','asc')
										   ->get('rb_penjualan_temp a')
										   ->result_array();
					}
	...
	public function detail(){
		...
		$dat = $this->db->query("SELECT * FROM rb_produk a where produk_seo='$ids' AND id_reseller!='0'");
	    $row = $dat->row();
	    $total = $dat->num_rows();
	        if ($total == 0){
	        	redirect('main');
	    ...
	    $data['varian'] = $this->db->select('a.nama_varian,a.id_varian')
	    					->where('a.id_produk',$row->id_produk)
	    					->get('rb_produk_varian a')
	    					->result();
	    //var_dump(($data['varian'])); exit;
